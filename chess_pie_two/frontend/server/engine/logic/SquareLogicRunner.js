...
class SquareLogicRunner {
  ...
  static execute(squareId, triggerType, context, board) {
    ...
    if (this.pendingTriggers.length > 0) {
      console.warn(
        "[SquareLogicRunner] Infinite loop risk detected. Clearing queue.",
      );
  ...
  static executeInternal(squareLogic, triggerType, context, board) {
    const triggers = squareLogic.logic.filter(
      (b) => b.type === "trigger" && b.id === triggerType,
    );
  ...
  static evaluateTriggerCondition(squareLogic, trigger, context, board) {
    ...
    switch (trigger.id) {
      case "on-step":
        ...
        return (
          matchesType(context.piece, vals.pieceType) &&
          matchesColor(context.piece, vals.pieceColor)
        );
    ...
    for (const [key, val] of Object.entries(socketValues)) {
      if (val && typeof val === "object" && val.type === "variable") {
        ...
        if (val.variableOnly || key === "varName") {
          ...
          resolved[key] =
            squareLogic.variables[varName] !== undefined
              ? squareLogic.variables[varName]
              : 0;
        }
      } else {
        resolved[key] = val;
