...
export class Piece {
    ...
    static create(id, type, color, position, rules = [], logic = [], name) {
        ...
        switch (lowerType) {
            case 'pawn':
                piece = new Pawn(id, color, position);
                break;
            case 'knight':
                piece = new Knight(id, color, position);
                break;
            case 'bishop':
                piece = new Bishop(id, color, position);
                break;
            case 'rook':
                piece = new Rook(id, color, position);
                break;
            case 'queen':
                piece = new Queen(id, color, position);
                break;
            case 'king':
                piece = new King(id, color, position);
                break;
            default:
                piece = new CustomPiece(id, type, color, position, actualRules, actualLogic, name);
                break;
        }
        if (name)
            piece.name = name;
    ...
    static createStandard(id, type, color, position) {
        const lowerType = type.toLowerCase();
        switch (lowerType) {
...
export class CustomPiece extends Piece {
    constructor(id, type, color, position, rules = [], logic = [], name) {
    ...
    isValidMove(from, to, board) {
        ...
        for (const rule of this.rules) {
            ...
            // Resolve a named quantity: built-in movement variable, else a piece variable
            const resolveNamed = (name) => {
                switch (name) {
                    case 'diffX': return dx;
                    case 'diffY': return forwardDy;
                    case 'absDiffX': return adx;
                    case 'absDiffY': return ady;
                    case 'dist':
                        if (board.gridType === 'hex') {
                            const ds = -dx - dy;
                            return (Math.abs(dx) + Math.abs(dy) + Math.abs(ds)) / 2;
                        }
                        return Math.max(adx, ady); // Chebyshev distance for square
                    default:
            ...
            for (let i = 0; i < rule.conditions.length; i++) {
                ...
                // Left side: an explicit named reference, otherwise the built-in movement variable
                const value = cond.variableVariable !== undefined
                    ? resolveNamed(cond.variableVariable)
                    : resolveNamed(cond.variable);
                // Right side: a named reference, otherwise the literal number
                const compareValue = cond.valueVariable !== undefined
                    ? resolveNamed(cond.valueVariable)
                    : cond.value;
                let condSatisfied = false;
                switch (cond.operator) {
                    case '===':
                        condSatisfied = value === compareValue;
                        break;
                    case '>':
                        condSatisfied = value > compareValue;
                        break;
            ...
            if (ruleResult) {
                ...
                if (rule.result === 'allow') {
                    ...
                    if (rule.type === 'slide' || rule.type === 'run' || rule.type === 'running') {
    ...
    isValidMove(from, to, board) {
        if (board.gridType === 'hex') {
            ...
            for (let i = 1; i < dist; i++) {
                const sq = `${fq + i * sqDir.q},${fr + i * sqDir.r}`;
                if (board.getPiece(sq) !== null || !board.isActive(sq))
                    return false;
            }
            return true;
        }
...
export class King extends Piece {
    ...
    isValidMove(from, to, board) {
        if (board.gridType === 'hex') {
            ...
            if (dist === 1)
                return true;
            if (dist === 2 && (dq === dr || dq === -2 * dr || 2 * dq === -dr))
                return true;
            return false;
        }
        const fromCoords = toCoords(from);
