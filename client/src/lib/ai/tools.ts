...
const LOGIC_TOOLS: OllamaTool[] = [
  ...
  { type: 'function', function: { name: 'add_logic_block', description: 'Add block to canvas. Triggers→Effects chain. Sockets: on-is-captured{by}, on-captured{piece}, on-threat{by}, on-environment{condition}, on-var{varName,op,value}, cooldown{duration,unit}, transformation{target}, modify-var{varName,op,value}, kill{target}, explode{radius}, tell-user{message}.', parameters: { type: 'object', properties: { pieceId: { type: 'string' }, templateId: { type: 'string', enum: ['on-is-captured', 'on-captured', 'on-threat', 'on-move', 'on-environment', 'on-var', 'cooldown', 'transformation', 'modify-var', 'prevent', 'kill', 'explode', 'tell-user', 'win'] }, position: { type: 'object', properties: { x: { type: 'number' }, y: { type: 'number' } }, required: ['x', 'y'] }, socketValues: { type: 'object' } }, required: ['pieceId', 'templateId', 'position'] } } },
