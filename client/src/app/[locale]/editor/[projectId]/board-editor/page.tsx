...
export async function generateMetadata({ params }: { params: Promise<{ locale: string; projectId: string }> }) {
    ...
    const t = await getTranslations({ locale, namespace: 'SEO.BoardEditor' });
    const url = `https://chessperiment.app/${locale}/editor/${projectId}/board-editor`;
    return {
        title: t('title'),
...
export default async function BoardEditorPage({ params }: { params: Promise<{ locale: string; projectId: string }> }) {
